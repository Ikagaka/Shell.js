...
var SurfaceCanvas_1 = require("./SurfaceCanvas");
class SurfaceRender {
    ...
    constructor(arg) {
        if (arg == null) {
            arg = new SurfaceCanvas_1.default().loadFromCanvas(SurfaceUtil.createCanvas());
        }
    ...
    getSurfaceCanvas() {
        return new SurfaceCanvas_1.default().loadFromCanvas(SurfaceUtil.copy(this.cnv));
    }
    // [
    ...
    composeElement(canvas, type, x, y) {
        switch (type) {
            case "base":
                this.base(canvas);
                break;
    ...
    pna(pna) {
        ...
        var dataB = new Uint8ClampedArray(pna.pixels);
        for (var y = 0; y < pna.height; y++) {
            for (var x = 0; x < pna.width; x++) {
                var iA = x * 4 + y * this.cnv.width * 4; // baseのxy座標とインデックス
                var iB = x * 4 + y * pna.width * 4; // pnaのxy座標とインデックス
    ...
    //着せ替え・elementでは使用不可。
    move(x, y) {
        var srfCnv = new SurfaceCanvas_1.default().loadFromCanvas(SurfaceUtil.copy(this.cnv));
