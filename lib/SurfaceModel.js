...
"use strict";
const SR = require("./SurfaceRender");
...
class SerikoLayer extends Layer {
    constructor(patternID, background = false) {
        super();
...
class MayunaLayer extends Layer {
    constructor(visible, background = false) {
        super();
...
class Surface extends events_1.EventEmitter {
    constructor(div, scopeId, surfaceId, surfaceDefTree, config, cache) {
        super();
        this.element = div;
    ...
    play(animationId) {
        ...
        if (!(this.layers[animationId] instanceof SerikoLayer)) {
            ...
            let layer = this.layers[animationId];
    ...
    talk() {
        ...
        const hits = animations.filter((anim, animId) => anim.intervals.some(([interval, args]) => "talk" === interval && this.talkCount % args[0] === 0));
