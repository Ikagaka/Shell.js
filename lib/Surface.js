...
class Surface extends EventEmitter.EventEmitter {
    ...
    play(animationId, callback) {
        ...
        this.animationsQueue[animationId] = patterns.map((pattern, i) => () => {
            ...
            switch (type) {
                case "start":
                    var { animation_id } = pattern;
                    this.play(Number((/(\d+)$/.exec(animation_id) || ["", "-1"])[1]), nextTick);
                    return;
                case "stop":
                    var { animation_id } = pattern;
                    this.stop(Number((/(\d+)$/.exec(animation_id) || ["", "-1"])[1]));
                    setTimeout(nextTick);
                    return;
    ...
    talk() {
        ...
        this.talkCount++;
        const hits = animations.filter((anim) => anim.intervals.some(([interval, args]) => "talk" === interval) && this.talkCount % this.talkCounts[anim.is] === 0);
        hits.forEach((anim) => {
