...
var Surface = (function (_EventEmitter2) {
    ...
    function Surface(canvas, scopeId, surfaceId, shell) {
        ...
        $elm.on("contextmenu", function (ev) {
            return processMouseEvent(ev, "mouseclick", _this);
        });
        $elm.on("click", function (ev) {
            return processMouseEvent(ev, "mouseclick", _this);
        });
        $elm.on("dblclick", function (ev) {
            return processMouseEvent(ev, "mousedblclick", _this);
        });
        $elm.on("mousedown", function (ev) {
            return processMouseEvent(ev, "mousedown", _this);
        });
        $elm.on("mousemove", function (ev) {
            return processMouseEvent(ev, "mousemove", _this);
        });
        $elm.on("mouseup", function (ev) {
            return processMouseEvent(ev, "mouseup", _this);
        });
        var tid = 0;
        var touchCount = 0;
        ...
        $elm.on("touchend", function (ev) {
            processMouseEvent(ev, "mouseup", _this);
            processMouseEvent(ev, "mouseclick", _this);
            if (Date.now() - touchStartTime < 500 && touchCount % 2 === 0) {
                processMouseEvent(ev, "mousedblclick", _this);
            }
        });
        $elm.on("touchstart", function (ev) {
            touchCount++;
            touchStartTime = Date.now();
            processMouseEvent(ev, "mousedown", _this);
            clearTimeout(tid);
...
function processMouseEvent(ev, type, srf) {
    ...
    var cnv = ev.target; // Element、お前は今日からHTMLElementだ(ていうかcanvas)
    ...
    if (/^touch/.test(ev.type) && !!document.createTouchList) {
        ...
        var touch = document.createTouch(ev["view"], ev.target, 0, //identifier
        ev.pageX, ev.pageY, ev.screenX, ev.screenY); //force
        var touches = document.createTouchList(touch);
        // http://qiita.com/damele0n/items/dc312bbf66da1d46dd6f
        var initTouchEvent = TouchEvent.prototype["initTouchEvent"];
        var args;
        if (true) {
            args = [touches, touches, touches, ev.type, ev["view"], ev.screenX, ev.screenY, ev.clientX, ev.clientY, ev.ctrlKey, ev.altKey, ev.shiftKey, ev.metaKey // {Boolean}   metaKey
            ];
        } else if (false) {
                args = [ev.type, ev.cancelBubble, ev.cancelable, ev["view"], ev["detail"], ev.screenX, ev.screenY, ev.clientX, ev.clientY, ev.ctrlKey, ev.altKey, ev.shiftKey, ev.metaKey, touches, touches, touches, 0, 0 // {Number}    rotation
                ];
            } else if (false) {
                    args = [ev.type, ev.cancelBubble, ev.cancelable, ev["view"], ev["detail"], ev.ctrlKey, ev.altKey, ev.shiftKey, ev.metaKey, touches, touches, touches // {TouchList} changedTouches
                    ];
                }
