...
(function (cuttlebone) {
    ...
    class Surface {
        constructor(canvas, scopeId, surfaceId, shell) {
            this.element = canvas;
        ...
        initAnimation(anim) {
            ...
            switch (_interval) {
                case "sometimes":
                    SurfaceUtil.random(((callback) => { if (!this.destructed && !this.stopFlags[is]) {
                        this.play(is, callback);
                    } }), 2);
                    break;
                case "rarely":
                    SurfaceUtil.random(((callback) => { if (!this.destructed && !this.stopFlags[is]) {
                ...
                case "random":
                    ...
                    break;
                case "periodic":
                    SurfaceUtil.periodic(((callback) => { if (!this.destructed && !this.stopFlags[is]) {
                        this.play(is, callback);
                    } }), n);
                    break;
                case "always":
                    SurfaceUtil.always(((callback) => { if (!this.destructed && !this.stopFlags[is]) {
                        this.play(is, callback);
                    } }));
                    break;
                case "runonce":
        ...
        play(animationId, callback) {
            ...
            var lazyPromises = anim.patterns.map((pattern) => () => new Promise((resolve, reject) => {
                var { surface, wait, type, x, y, animation_ids } = pattern;
                switch (type) {
                    case "start":
                        this.play(animation_ids[0], () => resolve(Promise.resolve()));
                        return;
                    case "stop":
                    ...
                    case "alternativestart":
                        this.play(SurfaceUtil.choice(animation_ids), () => resolve(Promise.resolve()));
                        return;
                    case "alternativestart":
        ...
        yenE() {
            var animations = this.surfaceTreeNode.animations;
            var hits = animations.filter((anim) => anim.interval === "yen-e" && this.talkCount % this.talkCounts[anim.is] === 0);
            hits.forEach((anim) => {
                this.play(anim.is);
            });
        }
