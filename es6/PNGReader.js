...
class PNGReader {
     ...
     */
    unFilterNone(scanline, pixels, bpp, offset, length) {
     ...
     */
    unFilterSub(scanline, pixels, bpp, offset, length) {
        var i = 0;
        for (; i < bpp; i++)
            pixels[offset + i] = scanline[i];
        for (; i < length; i++) {
     ...
     * Up(x) = Raw(x) + Prior(x)
     */
    unFilterUp(scanline, pixels, bpp, offset, length) {
        var i = 0;
        var byte;
        var prev;
     ...
     */
    unFilterAverage(scanline, pixels, bpp, offset, length) {
        var i = 0;
        var byte;
        var prev;
        var prior;
     ...
     *  end
     */
    unFilterPaeth(scanline, pixels, bpp, offset, length) {
        var i = 0;
        var raw;
        var a;
        var b;
